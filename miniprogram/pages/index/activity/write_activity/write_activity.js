// miniprogram/pages/index/activity/write_activity/write_activity.js
wx.cloud.init()
...
Page({
  ...
  formSubmit: function (e) {
    ...
    var title = e.detail.value.title;//活动名称
    var location = e.detail.value.location;//活动地点
    var date = e.detail.value.date;//活动日期
    var time = e.detail.value.time;//活动时间
    console.log(time)
    var jianjie = e.detail.value.jianjie;//活动简介
    ...
    if (this.data.imgList.length) {
      ...
      for (var i = 0; i < this.data.imgList.length; i++) {
        ...
        wx.cloud.uploadFile({
          cloudPath: "images/activity_images/" + name_img,    //云存储路径
          ...
          success: res => {
            ...
            if (images_fileID.length === imageFiles.length) {
              //对数据库进行操作
              db.collection("activity_form").add({
                data: {
                  userPhoto: userPhoto,
